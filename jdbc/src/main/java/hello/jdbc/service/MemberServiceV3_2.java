...
//    private final PlatformTransactionManager transactionManager;
    ...
    public MemberServiceV3_2(PlatformTransactionManager transactionManager, MemberRepositoryV3 memberRepository) {
        this.transactionTemplate = new  TransactionTemplate(transactionManager); // 트랜잭션 매니저를 템플릿에 주입, 유연성 생각하여 매니저를 가져와서 주입
    ...
    public void accountTransfer(String fromId, String toId, int money) throws SQLException {
        ...
        transactionTemplate.executeWithoutResult((status) -> {
            ...
            try {
                ...
                throw new IllegalStateException(e); // 이게 언체크 예외.
            }
        });
        //트랜잭션 템플릿의 기본 동작은 다음과 같다.
        //비즈니스 로직이 정상 수행되면 커밋한다.
        //언체크 예외가 발생하면 롤백한다. 그 외의 경우 커밋한다. (체크 예외의 경우에는 커밋하는데, 이 부분은 뒤
        //에서 설명한다.)
        // 체크, 언체크 예외가 뭐지? => 공부 필요. 뒤에서 다룸.

        //아래 코드 트랜잭션을 try catch 공통 코드를 위처럼 간결하게 변경
