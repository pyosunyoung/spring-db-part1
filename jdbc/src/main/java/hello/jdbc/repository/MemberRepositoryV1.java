 ...
 */
//외부에서 DataSource 를 주입 받아서 사용한다. 이제 직접 만든 DBConnectionUtil 을 사용하지 않
//아도 된다.
...
public class MemberRepositoryV1 {
    ...
    private void close(Connection con, Statement stmt, ResultSet rs){ // connection해제, 만약 con에서 close오류가 나면 pstmt로 못넘어감 이런 오류 해결하기 위해 따로 분리
        ...
        //커넥션 풀링을 사용하면 close가 닫아주는게 아닌 해당 커넥션을 반환시켜 주는 것,물론 객체 인스턴스 주소는 각각 다름.
        // 즉 커넥션 풀링을 사용하면 동일한 커넥션을 계속 사용할 수 있다.
        // dirvermanager일 떈 계쏙 닫고 새로 커넥션 생성하고 이런것이 반복됨.
    }
